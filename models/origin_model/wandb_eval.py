...
def main(params):
    ...
    if model_name == "atkt":
        save_test_path = os.path.join(save_dir,
                                      model.emb_type + "_test_ratio" + str(ratio) + "_" + str(use_pred) + "_" + str(
                                          atkt_pad) + "_predictions.txt")
    else:
        save_test_path = os.path.join(save_dir, model.emb_type + "_test_ratio" + str(ratio) + "_" + str(
            use_pred) + "_predictions.txt")
    ...
    if model_name in que_type_models and model_name != "lpkt":
        ...
        dfinal = evaluate_splitpred_question(model, data_config, testf, model_name, save_test_path, use_pred, ratio,
                                             atkt_pad)
    for key in dfinal:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="saved_model/nips_task34_cdkt_qid_saved_model_42_0_0.2_200_0.001_0_1")
